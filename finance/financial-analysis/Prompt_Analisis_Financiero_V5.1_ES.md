# PROMPT PROFESIONAL V5.1 — PIPELINE DE RESEARCH FINANCIERO CON PDF Y SCRIPT BRIEF BAJO DEMANDA

> Cambios V5.1 frente a V5.0: (A) completitud obligatoria de secuencias — ninguna
> se omite en silencio; (B) atribución de opiniones también en la narrativa, no
> solo en la matriz; (C) el Script Brief clasifica indicador vs estrategia de forma
> inequívoca, mantiene la sección de no-repaint a nivel de requisitos (sin inyectar
> APIs) y avisa cuando conviene un indicador genérico parametrizable.
>
...
10. Auditoría final (incluye control de completitud y de atribución).
...
- Niveles de precio sueltos o zonas de referencia → **Indicador**.
...
8. LAGUNAS / DECISIONES ABIERTAS  (para que el usuario confirme antes de codificar)
   - [Si la fuente arroja varios símbolos, cada uno con niveles fijos, señalar
      aquí que es candidato a un INDICADOR GENÉRICO parametrizable (niveles como
      ajustes por símbolo, NO condicionales por ticker incrustados en el código),
      y dejar la decisión al usuario/Granja.]
   - ...
